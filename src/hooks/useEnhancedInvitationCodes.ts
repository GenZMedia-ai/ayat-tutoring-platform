...
export const useEnhancedInvitationCodes = () => {
  ...
  // Set up real-time subscription for invitation codes
  useEffect(() => {
    console.log('🔄 Setting up real-time subscription for invitation codes');
    
    const channel = supabase
      .channel('invitation-codes-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload) => {
          console.log('📡 Real-time invitation code update:', payload);
          
          // Invalidate queries to refresh data
          queryClient.invalidateQueries({ queryKey: ['invitation-codes'] });
          
          // Show toast for usage updates
          if (payload.eventType === 'UPDATE') {
            const oldUsed = payload.old?.used_count || 0;
            const newUsed = payload.new?.used_count || 0;
            const code = payload.new?.code;
            
            if (newUsed > oldUsed) {
              toast.info(`Invitation code ${code} was used (${newUsed}/${payload.new?.usage_limit})`);
            }
          }
        }
      )
    ...
    return () => {
      console.log('🔌 Cleaning up invitation codes real-time subscription');
