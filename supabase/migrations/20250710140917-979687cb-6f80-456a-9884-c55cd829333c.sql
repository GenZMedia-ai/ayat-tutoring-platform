-- COMPREHENSIVE PERFORMANCE & SECURITY FIX PLAN - PHASE 1 & 3 (FIXED)
...
CREATE INDEX IF NOT EXISTS idx_payment_links_status_type_created 
...
CREATE INDEX IF NOT EXISTS idx_payment_links_family_status 
ON public.payment_links(family_group_id, status) WHERE family_group_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_payment_links_expires_status 
...
CREATE INDEX IF NOT EXISTS idx_session_students_student_session 
ON public.session_students(student_id, session_id);

CREATE INDEX IF NOT EXISTS idx_session_students_session_student 
ON public.session_students(session_id, student_id);

-- Profiles Performance (User Management & Authentication)
CREATE INDEX IF NOT EXISTS idx_profiles_role_status_type 
...
CREATE INDEX IF NOT EXISTS idx_profiles_telegram_verified 
...
CREATE INDEX IF NOT EXISTS idx_teacher_availability_date_available_booked 
ON public.teacher_availability(date, is_available, is_booked, teacher_id);

CREATE INDEX IF NOT EXISTS idx_teacher_availability_teacher_date_time 
ON public.teacher_availability(teacher_id, date, time_slot);

-- Sales Followups Performance
CREATE INDEX IF NOT EXISTS idx_sales_followups_agent_date_completed 
ON public.sales_followups(sales_agent_id, scheduled_date, completed);

CREATE INDEX IF NOT EXISTS idx_sales_followups_student_completed_date 
ON public.sales_followups(student_id, completed, scheduled_date);

-- Trial Outcomes Performance
CREATE INDEX IF NOT EXISTS idx_trial_outcomes_student_outcome_date 
ON public.trial_outcomes(student_id, outcome, submitted_at);

CREATE INDEX IF NOT EXISTS idx_trial_outcomes_session_submitted 
...
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW public.teacher_performance_cache;
  REFRESH MATERIALIZED VIEW public.student_summary_cache;
  REFRESH MATERIALIZED VIEW public.payment_summary_cache;
  ...
  IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'teacher_stats_cache') THEN
    REFRESH MATERIALIZED VIEW public.teacher_stats_cache;
  END IF;
END;
$$;
