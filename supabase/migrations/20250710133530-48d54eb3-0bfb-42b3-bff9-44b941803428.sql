-- PHASE 2: DATA INTEGRITY PROTECTION
...
-- Add missing NOT NULL constraints where appropriate for referential integrity
...
-- 3. Add proper constraints for payment_links
-- Ensure payment links have valid student references
DO $$
BEGIN
  -- Add constraint to ensure payment_links student_ids array contains valid student IDs
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'payment_links_valid_students_check'
    AND table_name = 'payment_links'
  ) THEN
    -- We'll implement this check via a trigger since array constraints are complex
    -- Create function to validate student IDs in payment_links
    CREATE OR REPLACE FUNCTION validate_payment_link_students()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    SECURITY DEFINER
    SET search_path = ''
    ...
    CREATE TRIGGER validate_payment_link_students_trigger
      ...
      FOR EACH ROW
      EXECUTE FUNCTION validate_payment_link_students();
  END IF;
END $$;

-- 4. Add data consistency checks
-- Ensure trial dates/times are consistent between students and family_groups
CREATE OR REPLACE FUNCTION validate_family_trial_consistency()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
CREATE TRIGGER family_trial_consistency_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION validate_family_trial_consistency();
