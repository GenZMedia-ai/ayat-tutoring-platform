...
-- Phase 1: Fixed Backend Database Updates
-- Create internal schema for privileged helper functions
CREATE SCHEMA IF NOT EXISTS internal;

-- Create helper function with postgres ownership (works within existing system)
CREATE OR REPLACE FUNCTION internal.update_teacher_last_booked(p_teacher_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Grant execute permission to authenticated users (same pattern as existing functions)
GRANT EXECUTE ON FUNCTION internal.update_teacher_last_booked TO authenticated;

-- Fix the main booking function to use helper function instead of direct profile update
...
-- Phase 3: Security and Reliability Enhancements
-- Add idempotency protection
ALTER TABLE public.sessions ADD COLUMN IF NOT EXISTS idempotency_key UUID;
